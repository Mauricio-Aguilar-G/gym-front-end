...
<script>
  export default {
    ...
    methods: {
      ...
      editAdmin(item){
          this.axios.put(`/gym/admin/edit_admin/${item._id}`, item)
          .then(res =>{
      ...
      deleteAdmin(id){
          console.log(id);
          this.axios.delete(`/gym/admin/delete_admin/${id}`)
          .then(res=>{
              const index = this.desserts.findindex(item=>item._id===res.data._id)
              this.desserts.splice(index,1);
              this.editedIndex=this.desserts.indexOf(item);
              this.editedItem= Object.assign({},item);
              alert("Admin was deleted succesfully!");
              this.$router.push({ path:'/view_admin'});
          })
          .catch(e=>{
              console.log(e.response);
